...
`opencode/agents/trackr.md` is a **primary opencode agent** that converts natural language input into trackr task operations (add, tag, link, status, remove, etc.) — no confirmation, executes directly. It ships via `just install-opencode` (into `~/.config/opencode/agents/`) and defers to the trackr skill for full CLI detail. Keep its verb list and granularity rule consistent with the skill when commands or the milestone contract change.
